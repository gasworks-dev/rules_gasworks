...
def _gasworks_toolchain_impl(ctx):
    ...
    template_variables = platform_common.TemplateVariableInfo({
        "GASWORKS_BIN": target_tool_path,
    })
    default = DefaultInfo(
        files = depset(tool_files),
        runfiles = ctx.runfiles(files = tool_files),
    )
    gasworksinfo = GasworksInfo(
...
gasworks_toolchain = rule(
    implementation = _gasworks_toolchain_impl,
    attrs = {
    ...
    doc = """Defines a gasworks compiler/runtime toolchain.
